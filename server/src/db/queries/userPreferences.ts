    ...
    const result = await pool.query<UserPreferences>(
        `UPDATE user_preferences
         SET
            ...
            needs_attention_categories = COALESCE($22, needs_attention_categories),
            needs_attention_max_items = COALESCE($23, needs_attention_max_items),
            needs_attention_offer_due_days = COALESCE($24, needs_attention_offer_due_days),
            needs_attention_offer_overdue_days = COALESCE($25, needs_attention_offer_overdue_days),
            needs_attention_post_interview_stale_days = COALESCE($26, needs_attention_post_interview_stale_days),
            needs_attention_post_interview_follow_up_days =
                COALESCE($27, needs_attention_post_interview_follow_up_days),
            needs_attention_application_stale_days = COALESCE($28, needs_attention_application_stale_days),
            needs_attention_application_follow_up_days =
                COALESCE($29, needs_attention_application_follow_up_days)
         WHERE user_id = $1
         RETURNING
            application_job_statuses,
            application_show_notes,
